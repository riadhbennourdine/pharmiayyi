...
export const generateCaseStudyFromText = async (text: string, theme: string, system: string): Promise<CaseStudy> => {
    const prompt = `
      ...
      - "recommendations": {
        "mainTreatment": "Tableau de 10 chaînes de caractères (string[]) résumant le traitement principal en points clés. Chaque point doit être très détaillé et inclure : le nom du traitement en gras (ex: **Nom du traitement**), la posologie précise (ex: **Amoxicilline**: Posologie: 1g 2 à 3 fois par jour), la durée du traitement (ex: Durée: 7 jours), les principales précautions d'emploi, contre-indications, interactions médicamenteuses pertinentes, et des conseils de dispensation spécifiques pour le pharmacien ou le préparateur (ex: surveillance, conseils au patient sur les effets indésirables spécifiques, gestion des interactions). Si le texte source ne fournit pas de détails précis, génère des exemples de posologies, durées, précautions et conseils plausibles et couramment admis pour un cas d'étude pédagogique destiné à des professionnels. Ne te contente pas de dire 'selon notice' ou des conseils génériques pour le patient.",
        "associatedProducts": "Tableau de 10 chaînes de caractères (string[]) résumant les produits associés en points clés. Chaque point doit être très détaillé et inclure : le nom du produit en gras (ex: **Nom du produit**), la posologie précise, la durée d'utilisation, les principales précautions d'emploi, contre-indications, interactions pertinentes, et des conseils de dispensation spécifiques pour le pharmacien ou le préparateur. Si le texte source ne fournit pas de détails précis, génère des exemples de posologies, durées, précautions et conseils plausibles et couramment admis pour un cas d'étude pédagogique destiné à des professionnels. Ne te contente pas de dire 'selon notice' ou des conseils génériques pour le patient.",
